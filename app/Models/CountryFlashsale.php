...
class CountryFlashsale{
    ...
    public function insert( int $id, array $data )
    {
        ...
        foreach ($data as $key => $value) {
            $this->db->getReference( $this->dbname )->getChild( "{$id}/flashsale" )->getChild( $key )->set( $value );
        }
    ...
    {
        $this->db->getReference( $this->dbname )->getChild( "{$id}/flashsale" )->remove();
    }
}